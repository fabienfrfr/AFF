...
class DATA_MEANING():
    ...
    def SCORE(self):
        ...
        curve_list, std_list = [], []
        type_list = np.unique(self.DF.TYPE.values).tolist()
        # subplot list
        for v in type_list :
        ...
        EF.FAST_PLOT(curve_list,std_list,type_list, '', '','', yaxis=[0.4,0.9])
        ## distrib part
        DIST_SCORE = []
    ...
    def EVOLUTION(self, SHOW_TREE=False):
        ...
        N_ = np.log(node_size.reshape((self.NB_GEN,self.NB_P_GEN)).T)
